...
def db_statistics(per_frame_values):
    ...
    N_bins = 4
    ids = np.round(
        np.linspace(1, len(per_frame_values), N_bins + 1) + 1e-10) - 1;
    ...
    for technique in techniques:
        ...
        J, j_M, j_O, j_D, F, f_M, f_O, f_D, T, t_M = \
            zip(*Parallel(n_jobs=cfg.N_JOBS)(delayed(db_eval_sequence)(
                technique, sequence, inputdir, metrics) for sequence in
                                             sequences))
    ...
    with open(cfg.FILES.DB_BENCHMARK, 'r') as f:
        ...
        if cfg.EVAL_SET == 'paper':
            db_benchmark['techniques'] = filter(
                lambda technique: technique.eval_set == 'paper',
                db_benchmark['techniques'])
        return db_benchmark
